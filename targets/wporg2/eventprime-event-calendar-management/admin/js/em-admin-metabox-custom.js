...
$('#em_start_time, #em_start_date, #em_end_date').on('change', updateEndTimePicker);
   ...
   // add class to ui time picker ui-timepicker-wrapper
        ...
        $(form).submit(function (e) {
            ...
            if( em_end_date_timestamp < em_start_date_timestamp ) {
                show_toast( 'error', em_event_meta_box_object.end_date_less_from_start );
                // document.getElementById( 'em_end_date' ).focus();
                formError = 1;
            }
        ...
        $( document ).on( 'click', '#ep_event_booking_turn_off_continue', function() {
            ...
            $( '#ep_event_booking_turn_off_modal' ).closePopup({
                anim: (!$(this).attr('data-animation') || $(this).data('animation') == null) ? 'ep-modal-' : $(this).data('animation')
    ...
    $( document ).on( 'click', '.ep_event_metabox_tabs li a', function(e) {
        ...
        if( $( "#"+panelSrc ).length > 0 ) {
            ...
            if( panelSrc == 'ep_event_checkout_fields_data' ) {
                ...
                $('#ep_event_booking_fields_data').hide();
                $('.ep_event_booking_fields_wrap a').removeClass( 'ep-tab-active' );
                $('.ep_event_attendee_fields_wrap a').addClass( 'ep-tab-active' );
            }
        }
    });
...
//    });
    ...
    function validateDateField( dateInput ) {
        if ( dateInput == '' ) {    // If the date field is empty no validation required as per current workflow. 
            return true;
        }
        ...
        const regex = dateRegexMap[date_format]; 
    ...
    $( document ).on( 'click', '#ep_save_ticket_tier', function() {
        ...
        if( em_event_ticket_qty ) {
            ...
            if( max_qty ) {
                ...
                // Only check when ticket within category
    ...
    function initiate_the_ticket_modal(initiate=false){
        ...
        $( 'input[name=offers]' ).val( '' );
         $("#ep_ticket_start_booking_type").prop('selectedIndex', 0);
         $("#ep_ticket_ends_booking_type").prop('selectedIndex', 0);
         $("#ep_ticket_start_booking_type").trigger( 'change' );
         $("#ep_ticket_ends_booking_type").trigger( 'change' );
        initiate_the_offer_section();
    ...
    $(document).ready(function() {
        var ep_link_form_element = $('<form>', {
            id: 'wp-link',
            tabindex: '-1',
        });
        // Wrap the content inside #wp-link-wrap with the form element
