jQuery( function( $ ) {
    ...
    $( document ).on( 'click', '#ep_save_checkout_field', function() {
        ...
        $.ajax({
            ...
            success: function(data, textStatus, XMLHttpRequest){
                ...
                $( '#ep_event_settings_checkout_fields_container #ep_modal_buttonset .ep-checkout-field-modal-close' ).before('<span class="ep-success-message ep-mr-3">'+field_data_message+'</span>');
                setTimeout( function() {
    ...
    function ep_submit_payment_ajax( ele ) {
        ...
        // Check if 'action' key already exists in formData
        if (formData.has('action')) {
            formData.delete('action'); // Remove existing 'action' key
        }
